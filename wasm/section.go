...
func (m *Module) readSection(r *readpos.ReadPos) (bool, error) {
	...
	payloadDataLen, _, err := leb128.ReadVarUint32Size(r)
	if err != nil {
		return false, err
	}
...
func (m *Module) readNames(sec *SectionCustom) error {
	r := bytes.NewReader(sec.Data)

	for {
		nameType, err := leb128.ReadVarUint32(r)
		if err != nil {
			return err
		}
		...
		num, err := leb128.ReadVarUint32(r)
		if err != nil {
			return err
		}
...
func (s *SectionCode) ReadPayload(r io.Reader) error {
	count, encSize, err := leb128.ReadVarUint32Size(r)
	if err != nil {
		return err
	}
