...
import (
	...
	"github.com/evanphx/columbia/wasm/internal/stack"
...
type Instr struct {
	...
	// The offset of the instruction within the code section. This is used
	// later for correlating it with the dwarf debugging.
	Offset int64
...
func NewDisassembly(fn wasm.Function, module *wasm.Module) (*Disassembly, error) {
	code := fn.Body.Code
	instrs, err := Disassemble(code, fn.Body.Loc)
	if err != nil {
		return nil, err
	}
	disas := &Disassembly{}
...
func Disassemble(code []byte, offset int64) ([]Instr, error) {
	...
	for {
		...
		switch op {
		case ops.Block, ops.Loop, ops.If:
			sig, sz, err := leb128.ReadVarint32Size(reader)
			if err != nil {
				return nil, err
			}
		...
		case ops.Br, ops.BrIf:
			depth, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
		...
		case ops.BrTable:
			targetCount, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
			...
			for i := uint32(0); i < targetCount; i++ {
				entry, sz, err := leb128.ReadVarUint32Size(reader)
				if err != nil {
					return nil, err
				}
				offset += int64(sz)
			...
			defaultTarget, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
		...
		case ops.Call, ops.CallIndirect:
			index, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
			...
			if op == ops.CallIndirect {
				reserved, sz, err := leb128.ReadVarUint32Size(reader)
				if err != nil {
					return nil, err
				}
				offset += int64(sz)
		...
		case ops.GetLocal, ops.SetLocal, ops.TeeLocal, ops.GetGlobal, ops.SetGlobal:
			...
			instr.Immediates = append(instr.Immediates, index)
		case ops.I32Const:
			i, sz, err := leb128.ReadVarint32Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
		...
		case ops.I64Const:
			i, sz, err := leb128.ReadVarint64Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
		...
		case ops.I32Load, ops.I64Load, ops.F32Load, ops.F64Load, ops.I32Load8s, ops.I32Load8u, ops.I32Load16s, ops.I32Load16u, ops.I64Load8s, ops.I64Load8u, ops.I64Load16s, ops.I64Load16u, ops.I64Load32s, ops.I64Load32u, ops.I32Store, ops.I64Store, ops.F32Store, ops.F64Store, ops.I32Store8, ops.I32Store16, ops.I64Store8, ops.I64Store16, ops.I64Store32:
			...
			flags, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
			...
			ioffset, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
			instr.Immediates = append(instr.Immediates, ioffset)
		case ops.CurrentMemory, ops.GrowMemory:
			res, sz, err := leb128.ReadVarUint32Size(reader)
			if err != nil {
				return nil, err
			}
			offset += int64(sz)
