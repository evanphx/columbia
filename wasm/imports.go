...
import (
	...
	"github.com/evanphx/columbia/wasm/leb128"
)
...
type ImportEntry struct {
	...
	Name string // name as held by the name section
...
func (module *Module) resolveImports(resolve ResolveFunc) error {
	...
	for _, importEntry := range module.Import.Entries {
		...
		case ExternalFunction:
			...
			importIndex := importEntry.Type.(FuncImport).Type
			if len(fn.Sig.ReturnTypes) != len(module.Types.Entries[importIndex].ReturnTypes) {
				return InvalidImportError{importEntry.ModuleName, importEntry.FieldName, importIndex, fmt.Sprintf("wrong return value count: %d != %d", len(fn.Sig.ReturnTypes), len(module.Types.Entries[importIndex].ReturnTypes))}
			}
			if len(fn.Sig.ParamTypes) != len(module.Types.Entries[importIndex].ParamTypes) {
				return InvalidImportError{importEntry.ModuleName, importEntry.FieldName, importIndex, fmt.Sprintf("wrong param value count: %d != %d", len(fn.Sig.ParamTypes), len(module.Types.Entries[importIndex].ParamTypes))}
			}
			for i, typ := range fn.Sig.ReturnTypes {
				if typ != module.Types.Entries[importIndex].ReturnTypes[i] {
					return InvalidImportError{importEntry.ModuleName, importEntry.FieldName, importIndex, fmt.Sprintf("return value %d: wrong type %s", typ)}
				}
			}
			for i, typ := range fn.Sig.ParamTypes {
				if typ != module.Types.Entries[importIndex].ParamTypes[i] {
					return InvalidImportError{importEntry.ModuleName, importEntry.FieldName, importIndex, fmt.Sprintf("param value %d: wrong type %s", typ)}
				}
			}
			module.FunctionIndexSpace = append(module.FunctionIndexSpace, *fn)
