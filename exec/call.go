...
func (vm *VM) callIndirect() {
	...
	// Allow the caller to pass more arguments that the function can take. We
	// just pop the extra ones early so the function doesn't get confused.
	if len(fnActual.Sig.ParamTypes) < len(fnExpect.ParamTypes) {
		extra := len(fnExpect.ParamTypes) - len(fnActual.Sig.ParamTypes)
		for i := 0; i < extra; i++ {
			vm.popInt32()
		}
	}
	...
	*/
	...
	for i := range fnActual.Sig.ParamTypes {
